// Add this import to the top of inventory.js
import { enablePlayerControls, disablePlayerControls } from '../commonFiles/playerController.js';
...
let currentSelectedSlot = null;
    ...
    let inventoryData = {
        capacity: { current: 51, max: 250 },
        items: [
          { id: 1, name: 'Resistor', icon: 'resistor.png', count: 1, category: 'weapon', description: 'Used to control or limit the flow of electrical current in a circuit', stats: '70/140 BR' },
          { id: 2, name: 'Battery', icon: 'battery.png', count: 44, category: 'ammo', description: 'Stores chemical energy and converts it into electrical energy to power electronic devices. ', stats: '44/100' },
          { id: 3, name: 'Led', icon: 'led.png', count: 1, category: 'consumable', description: 'semiconductor that emits light when an electric current passes through it', stats: 'HP +50' },
          { id: 4, name: 'Jumper wire', icon: 'jumper_wire.png', count: 3, category: 'weapon', description: 'used to connect components on a breadboard or within a circuit for prototyping or testing without soldering', stats: 'Damage: 50' },
          { id: 5, name: 'Capacitor', icon: 'capacitor.png', count: 4, category: 'consumable', description: ' used to store electrical energy in an electric field and then release it back into the circuit', stats: 'HP +30' },
          { id: 6, name: 'Diode', icon: 'diode.png', count: 1, category: 'weapon', description: 'It acts as a one-way switch, allowing current to flow in one direction but blocking it in the reverse direction', stats: 'Damage: 20' },
          { id: 7, name: 'Transistor', icon: 'transistor.png', count: 1, category: 'key', description: 'A transistor is a semiconductor device that can function as an electronic switch or an amplifier for electrical signals', stats: 'Access Level 1' },
...
function createCircuitSlot(item, index) {
  ...
  // SVG for circuit board style slot
  ...
  <svg width="100%" height="100%" viewBox="0 0 60 60" xmlns="http://www.w3.org/2000/svg">
    ...
    <foreignObject x="8" y="8" width="44" height="44">
      <div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;">
        <img src="./assets/${item.icon}" style="width:80%;height:80%;object-fit:contain;filter:drop-shadow(0 0 4px rgba(0,255,255,0.5));" alt="" />
      </div>
    </foreignObject>
...
`;
  ...
  capacitySpan.textContent = `Capacity ${inventoryData.capacity.current}/${inventoryData.capacity.max}`;
  ...
  largeImg.src = `./assets/resistor1.png`; // Default
  largeImg.alt = '';
...
function selectSlot(event, slot) {
  ...
  if (item) {
    document.querySelector('.item-large img').src = `./assets/${item.icon}`;
...
export function addItem(item) {
  ...
  inventoryData.capacity.current += 1;
...
function updateCapacityDisplay() {
  const cap = document.querySelector('.top-bar span:nth-last-child(2)');
  if (cap) cap.textContent = `Capacity ${inventoryData.capacity.current}/${inventoryData.capacity.max}`;
}
