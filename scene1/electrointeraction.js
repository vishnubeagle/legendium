...
const ELECTRO_POSITION = {
 initial: { x: 22, y: 50, z: 78 },
 final: { x: -14, y: 9.8, z: -12 },
 rotation: { x: 0, y: -Math.PI / 2, z: 0 }
};

const UFO_PATH = {
 start: { x: 10, y: 40, z: 78 },
 end: { x: -14, y: 24, z: -12 },
 arcAmount: 8,
 scale: 0.1
};
...
function createSpotlight() {
 ...
 spotlight.position.set(-14, 15, -12);
 spotlight.target.position.set(-14, 0, -12);
 scene.add(spotlight);
...
function startUFOSequence() {
 ...
 if (renderer) {
  ...
  hologramMesh.position.set(-14, 23.5, -12);
...
function completeElectroSequence() {
  ...
  // Move electro to (22, 0, 78)
  if (electroComponents?.electro) {
   electroComponents.electro.position.set(22, 0, 78);
  }
