...
export async function initializeScene5(existingRenderer, isVRMode) {
  ...
  // After unoModel is loaded and unoMeshes is populated, ensure this event is registered:
  // renderer.domElement.addEventListener('pointerdown', onUnoPointerDown);

  // function onUnoPointerDown(event) {
  //   console.log('[DEBUG] onUnoPointerDown fired');
  //   if (!unoIsNearCamera || !unoMeshes.length) return;
  //   const rect = renderer.domElement.getBoundingClientRect();
  //   const mouse = new THREE.Vector2(
  //     ((event.clientX - rect.left) / rect.width) * 2 - 1,
  //     -((event.clientY - rect.top) / rect.height) * 2 + 1
  ...
  //   const raycaster = new THREE.Raycaster();
  //   raycaster.setFromCamera(mouse, camera);
  //   const intersects = raycaster.intersectObjects(unoMeshes, true);
  //   if (intersects.length > 0) {
  //     const mesh = intersects[0].object;
  //     console.log('[DEBUG] Mesh clicked:', mesh.name, mesh);
  //     if (mesh.name === "ArduinoBlue2_Back_0") return; // Skip this mesh
  //     showMeshUIPanel(mesh);
  //     selectedUnoMesh = mesh;
  //   } else {
  //     hideMeshUIPanel();
  //     selectedUnoMesh = null;
  //   }
  // }

  // function createUnoHolographicOutline() {
  //   if (!unoModel) return;
  //   
  //   // Clone the UNO model geometry for the outline
  //   unoModel.traverse((child) => {
  //     if (child.isMesh) {
  //       // Create a slightly larger geometry for the outline
  //       const geometry = child.geometry.clone();
  //       const scale = 1.01; // Make outline 10% larger
  //       
  //       // Scale the geometry
  //       const positions = geometry.attributes.position;
  //       for (let i = 0; i < positions.count; i++) {
  ...
  //       outlineMesh.scale.copy(child.scale);
  //       
  //       // Add outline to the UNO model
  //       unoModel.add(outlineMesh);
  //       
  //       // Store reference to the outline
  //       if (!unoOutline) {
  //         unoOutline = outlineMesh;
  //     }
  //   }
  //   });
  // }

  // Setup EffectComposer for background blur
  // composer = new EffectComposer(renderer);
  // renderPass = new RenderPass(scene, camera);
  // composer.addPass(renderPass);
  // blurPass = new UnrealBloomPass(
  //   new THREE.Vector2(window.innerWidth, window.innerHeight),
  //   0.7, // strength (higher = more blur)
  //   0.4, // radius
  //   0.85 // threshold
  // );
  // blurPass.enabled = false; // Only enable when UNO is near camera
...
export function cleanupScene5() {
  ...
  // Clean up uno outline
  // if (unoOutline) {
  //   if (unoOutline.geometry) unoOutline.geometry.dispose();
  //   if (unoOutline.material) unoOutline.material.dispose();
  //   unoOutline = null;
  // }
