...
function animateComponentToOriginal(key) {
  ...
  if (controls && !isComponentIntroSequencePlaying) controls.enabled = true;
  const closeButton = document.getElementById('closeUnoButton');
...
if (!camera || (!camera.isPerspectiveCamera && !camera.isOrthographicCamera)) return;
  ...
  startBuildButtonCleanup = function() {
...
renderer.domElement.addEventListener('pointerdown', event => {
  // 💥 Safety guard: prevent any processing if scene5 is being cleaned up or camera is gone
  if (
  ...
  // Prevent invalid raycasts during cleanup or scene switch
  try {
    const rect = renderer.domElement.getBoundingClientRect();
    if (!rect.width || !rect.height) return; // Avoid NaN in normalized coords
    ...
    // Prevent any crash during rapid scene switching
...
export function cleanupScene5() {
  ...
  componentModels = {}; // Clear the object
  focusedComponentKey = null;
