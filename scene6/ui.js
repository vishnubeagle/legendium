...
import { KpRgbDefault } from "./kpRgbLesson.js";
import { scene } from "./scene6.js";
import { KpMotorLesson } from "./kpMotorLesson.js";
import { KpIRLesson } from "./kpIRLesson.js";
...
export function updateNextButtonState() {
  ...
  if (currentLesson === "lesson1") {
    ...
    if (step === 0) {
      ...
      try {
        if (!window._lesson1StartCodingAudioPlayed) {
          playAudio("lesson1_s5");
          window._lesson1StartCodingAudioPlayed = true;
        }
    ...
    if (step === 0) {
      ...
      console.log("Last step - showing runCodeButton");
      try {
        if (!window._lesson3StartCodingAudioPlayed) {
          playAudio("lesson3_s5");
          window._lesson3StartCodingAudioPlayed = true;
        }
...
export function setLesson(lessonId) {
  if (lessonSteps[lessonId]) {
    ...
    console.log("🎯 Window currentLessonId before transition:", window.currentLessonId);
    ...
    // Play lesson2_s8 audio specifically when moving from lesson2 to lesson3
    if (currentLessonId === "lesson2" && lessonId === "lesson3") {
      console.log("🎵 DETECTED lesson2->lesson3 transition - attempting to play lesson2_s8");
      
      // Debug: Check allAssets structure
      console.log("🎵 allAssets object:", allAssets);
      console.log("🎵 allAssets.audios:", allAssets?.audios);
      
      try {
        console.log("🎵 Calling playAudio('lesson2_s8')");
        const audioResult = playAudio('lesson2_s8');
        console.log("🎵 playAudio result:", audioResult);
        
        // Check if audio exists in allAssets
        if (allAssets && allAssets.audios) {
          console.log("🎵 Available audios:", Object.keys(allAssets.audios));
          console.log("🎵 lesson2_s8 audio object:", allAssets.audios['lesson2_s8']);
          
          // Check if the audio file path is correct
          if (allAssets.audios['lesson2_s8']) {
            console.log("🎵 Audio object details:", {
              hasBuffer: !!allAssets.audios['lesson2_s8'].buffer,
              isPlaying: allAssets.audios['lesson2_s8'].isPlaying,
              volume: allAssets.audios['lesson2_s8'].getVolume ? allAssets.audios['lesson2_s8'].getVolume() : 'N/A'
            });
          } else {
            console.log("❌ lesson2_s8 is undefined in allAssets.audios");
        ...
        console.error("❌ Failed to play lesson2_s8 audio during lesson2->lesson3 transition:", e);
      }
    }
    ...
    console.log("🎯 After transition - currentLessonId:", currentLessonId, "window.currentLessonId:", window.currentLessonId);
    // Override global getters to avoid stale overrides from lessons
    try {
      window.getCurrentLesson = function () { return currentLessonId; };
      window.getCurrentStep = function () { return currentStep; };
    } catch (e) {}
    currentStep = 0;
    try { window._lesson1ContinueAudioPlayed = false; } catch (e) {}
    try { window._lesson1StartCodingAudioPlayed = false; } catch (e) {}
    try { window._lesson3StartCodingAudioPlayed = false; } catch (e) {}
    try { window._lesson4S6TransitionSetUp = false; } catch (e) {}
    try { window._lesson5AudioSequenceSetUp = false; } catch (e) {}
    try { window._lesson5ContinueAudioPlayed = false; } catch (e) {}
    if (stepTextBlock) {
    ...
    try {
      if (typeof window.setCodeEditorLesson === 'function') {
        window.setCodeEditorLesson(lessonId);
      }
    } catch (e) { console.warn('setCodeEditorLesson call failed:', e); }

    // 🎯 CALL YOUR FUNCTION HERE - SPECIFIC LESSON STARTED!
    if (lessonId === "lesson3") {
      console.log("Starting lesson3 with scene:", scene, "camera:", window.camera);
      try {
        if (!scene || !window.camera) {
          console.error("Scene or camera not available for lesson3");
          return;
        }
        // KpMotorLesson(scene, window.camera);
        KpRgbDefault(scene, window.camera);
     // KpIRLesson(scene, window.camera);
        // KpIRLesson(scene,window.camera)
    ...
    if (lessonId === "lesson4") {
      console.log("Starting lesson4 with scene:", scene, "camera:", window.camera);
      try {
        if (!scene || !window.camera) {
          console.error("Scene or camera not available for lesson4");
          return;
        }
        if (!window._motorLessonInitialized) {
         // KpMotorLesson(scene, window.camera);
          window._motorLessonInitialized = true;
    ...
    if (lessonId === "lesson5") {
      console.log("Starting lesson5 with scene:", scene, "camera:", window.camera);
      try {
        if (!scene || !window.camera) {
          console.error("Scene or camera not available for lesson5");
          return;
        }
        if (!window._irLessonInitialized) {
         // KpIRLesson(scene, window.camera);
          window._irLessonInitialized = true;
...
const codePlaneText = new ThreeMeshUI.Text({
  content: "", // Start with empty content so nothing is visible before reveal
...
export function updateCodePlaneWithInstruction(text) {
  if (codePlaneText && codePlane) {
    ...
    try {
      if (typeof window !== 'undefined' && typeof window.hideQuestionsPanel === 'function') {
        window.hideQuestionsPanel();
      }
    } catch (e) {
...
export function showNextLessonButton() {
  ...
  try { if (window.codeEditorGroup) window.codeEditorGroup.visible = true; } catch (e) {}
  // Rename button for lesson5
  try {
    const isLesson5 = typeof window.getCurrentLesson === 'function' && window.getCurrentLesson() === 'lesson5';
...
function attachNextLessonHandler() {
  if (!window._nextLessonRaycastHandler) {
    window._nextLessonRaycastHandler = (event) => {
      const mouse = new THREE.Vector2(
        (event.clientX / window.innerWidth) * 2 - 1,
        -(event.clientY / window.innerHeight) * 2 + 1
      );
      const raycaster = new THREE.Raycaster();
      const camera = window.camera || window._sceneCamera;
      if (!camera) return;
    ...
    window.addEventListener("pointerdown", window._nextLessonRaycastHandler);
  }
}
function detachNextLessonHandler() {
  if (window._nextLessonRaycastHandler) {
    window.removeEventListener("pointerdown", window._nextLessonRaycastHandler);
    window._nextLessonRaycastHandler = null;
  }
}
...
// --- Show Blinking Button ---
export const showBlinkingButton = new ThreeMeshUI.Block({
  width: 0.8,
  height: 0.15,
  justifyContent: "center",
  contentDirection: "row",
  ...
  new ThreeMeshUI.Text({
    content: "Show blinking",
  })
);
showBlinkingButton.position.set(0.2, 1.9, -4.01);
showBlinkingButton.visible = false; // Start hidden
showBlinkingButton.userData.clickable = true;
showBlinkingButton.traverse((child) => {
  child.userData.clickable = true;
  child.userData.parentButton = showBlinkingButton;
});
...
export function disableCameraAnimation() {
  window.cameraAnimationDisabled = true;
}

export function enableCameraAnimation() {
  window.cameraAnimationDisabled = false;
}
...
export function resetAllModelsToOriginalPosition() {
  ...
  if (window.expansionBoardModel) {
    window.expansionBoardModel.position.copy(
      modelTransforms.expansionBoard.position
    );
    window.expansionBoardModel.rotation.copy(
  ...
  if (window.rgbLEDModel && window.currentScene) {
    window.currentScene.remove(window.rgbLEDModel);
    // Reset RGB LED rotation to original position
    window.rgbLEDModel.rotation.copy(modelTransforms.rgbLED.rotation);
    // Optionally, also set window.rgbLEDModel = null; if you want to fully clear the reference
    // window.rgbLEDModel = null;
  }
  // JST Pin(s)
  if (window.jstPin) {
    if (window.jstPin.pinGLTF1) {
      window.currentScene.remove(window.jstPin.pinGLTF1);
    }
    if (window.jstPin.pinGLTF2) {
      window.currentScene.remove(window.jstPin.pinGLTF2);
    }
  }
...
function removeLesson2Models() {
  ...
  // Dispose all wires
  if (window.jstPin && window.jstPin.wires) {
...
export function removeAllLesson2Models() {
  ...
  // Remove nano model
  if (window.nanoModel && window.currentScene) {
    console.log("Removing nanoModel from scene");
    window.currentScene.remove(window.nanoModel);
  } else {
    console.log("nanoModel not found or no scene available");
  }

  // Remove expansion board model
  if (window.expansionBoardModel && window.currentScene) {
    console.log("Removing expansionBoardModel from scene");
    window.currentScene.remove(window.expansionBoardModel);
  } else {
...
export function removeAllLesson3Models() {
  ...
  if (window.tempSensorModel && window.currentScene) {
    console.log("Removing tempSensorModel from scene");
    window.currentScene.remove(window.tempSensorModel);
  } else {
    console.log("tempSensorModel not found or no scene available");
  }

  // Remove jstPin3 (if exists)
  if (window.jstPin3 && window.jstPin3.group && window.currentScene) {
    console.log("Removing jstPin3 from scene");
    window.currentScene.remove(window.jstPin3.group);
  } else {
...
window.getCurrentLesson = function () {
...
window.addEventListener('audioComplete-lesson3_s8', () => {
...
window.addEventListener('audioComplete-lesson3_s6', () => {
...
window.addEventListener('audioComplete-lesson3_s9', () => {
...
window.addEventListener('audioComplete-lesson3_s10', () => {
...
window.addEventListener('audioComplete-lesson4_s6', () => {
...
window.addEventListener('audioComplete-lesson4_s7', () => {
...
window.addEventListener('audioComplete-lesson5_s8', () => {
...
window.addEventListener('audioComplete-lesson5_s9', () => {
  console.log("🎵 GLOBAL EVENT: audioComplete-lesson5_s9 fired!");
});

window.addEventListener('audioComplete-lesson5_s10', () => {
  console.log("🎵 GLOBAL EVENT: audioComplete-lesson5_s10 fired!");
});

window.addEventListener('audioComplete-lesson5_s11', () => {
