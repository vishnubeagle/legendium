// Learning Panel - MeshUI Panel for Start Coding
...
import questionsLesson1 from "./lesson1-questions.json";
import questionsLesson2 from "./lesson2-questions.json";
import questionsLesson3 from "./lesson3-questions.json";
import questionsLesson4 from "./lesson4-questions.json";
import questionsLesson5 from "./lesson5-questions.json";
...
// Track lesson content state
let currentLessonName = null;
let currentLessonIndex = 0;
let learningSceneRef = null;

// Section containers for bordered display
...
// Function to create the learning panel
export function createLearningPanel(scene) {
  ...
  try {
    if (typeof window !== "undefined" && window.camera) {
      attachLearningArrowHandler(window.camera);
      attachLearningQuizHandler(window.camera);
    }
  } catch (e) {}
...
function createQuestionsPanel(scene) {
  ...
  // Also check window.getCurrentLesson() as a fallback to ensure we have the correct lesson
  const actualLesson =
    currentLessonName ||
  ...
  if (actualLesson === "lesson2") {
    ...
    quizData = questionsLesson4 || null;
    console.log(
    ...
    quizData = questionsLesson5 || null;
    console.log(
    ...
    quizData = questionsLesson1 || null;
  ...
  try {
    if (typeof window !== "undefined" && window.camera) {
      attachQuestionsHandler(window.camera);
    }
  } catch (e) {}
...
if (typeof window !== "undefined") {
  window.hideQuestionsPanel = hideQuestionsPanel;
}
...
function updateQuestionsPanelContent() {
  ...
  const currentLessonNameSafe = (typeof window !== 'undefined' && typeof window.getCurrentLesson === 'function')
    ? window.getCurrentLesson()
      ...
      if (elapsed < durationMs) {
              ...
              try {
                if (!window._lesson4_s8Played) {
                  ...
                  if (playAudio) {
                    ...
                    window._lesson4_s8Played = true; // mark only after playAudio call
                    ...
                    const onL4S9Complete = () => {
                      ...
                      try {
                        if (typeof window.showNextLessonButton === "function")
                          window.showNextLessonButton();
        ...
        try {
          ...
          if (isLesson5) {
            ...
            try {
              if (typeof window !== "undefined" && !window._lesson5_s10Played) {
                const exists =
                  typeof allAssets !== "undefined" &&
                  allAssets.audios &&
                ...
                window._lesson5_s10Played = true;
              }
            } catch (e) {
              ...
              const onL5S10Complete = () => {
                ...
                try {
                  if (!window._lesson5_s11Played) {
                    ...
                    window._lesson5_s11Played = true;
                  }
                } catch (e) {
        ...
        try {
          if (typeof window !== "undefined") {
            if (typeof window.applyRGBLEDBlinkShader === "function") {
              window.applyRGBLEDBlinkShader();
            }
            if (typeof window.controlRGBLEDBlink === "function") {
              window.controlRGBLEDBlink("start");
            }
          }
        } catch (e) {}
        // Play the corresponding audio cue while the LED starts blinking
        try {
          if (currentLessonName === "lesson2") {
            playAudio && playAudio("lesson2_s6");
          } else if (currentLessonName === "lesson3") {
        ...
        // For lesson2: after lesson2_s6 completes, play lesson2_s7
        try {
          if (
            typeof window !== "undefined" &&
            ...
            const onL2S6Complete = () => {
              ...
              // After 'lesson2_s7' ends, play 'lesson2_s8'
              try {
          ...
          if (
            ...
            const onL2S8Complete = () => {
              try {
                ...
                if (window.codeEditorGroup)
                  window.codeEditorGroup.visible = true;
                if (typeof window.showNextLessonButton === "function")
                  window.showNextLessonButton();
                if (typeof window.setForwardArrowEnabled === "function")
                  window.setForwardArrowEnabled(false);
...
function attachQuestionsHandler(camera) {
  ...
  if (!questionsCamera) {
    console.warn(
      "[Learning] attachQuestionsHandler: camera not provided and window.camera not found"
    );
  }
