...
export class ShaderManager {
  ...
  getDragLabel() {
    ...
    if (!this._dragLabel) {
      ...
      try {
        if (window.currentScene) window.currentScene.add(this._dragLabel);
      } catch (e) {}
  ...
  getDropLabel() {
    ...
    if (!this._dropLabel) {
      ...
      try {
        if (window.currentScene) window.currentScene.add(this._dropLabel);
      } catch (e) {}
  ...
  getSnapPointFor(draggedObjectName) {
    ...
    const lesson = typeof window.getCurrentLesson === 'function' ? window.getCurrentLesson() : 'lesson1';
    const step = typeof window.getCurrentStep === 'function' ? window.getCurrentStep() : 0;
    ...
    if (lesson === 'lesson3') {
      ...
      if (step === 3 && (draggedObjectName === 'jstPin2' || draggedObjectName === 'jstPin2.pinGLTF2')) {
        ...
        try {
          if (window.ldrModel) {
            window.ldrModel.traverse((child) => {
    ...
    if (lesson === 'lesson4') {
      ...
      if (step === 2 && (draggedObjectName === 'jstPin' || draggedObjectName === 'jstPin.pinGLTF1')) {
        const motorDriver = typeof window !== 'undefined' ? window.motorDriverModel : null;
        try {
  ...
  getTargetObject(targetName) {
    ...
    console.log('Shader Manager: Available objects:', {
      jstPin: window.jstPin,
      jstPinGroup: window.jstPin?.group,
      jstPin2: window.jstPin2,
      jstPin3: window.jstPin3,
      nanoModel: window.nanoModel,
      tempSensorModel: window.tempSensorModel,
      expansionBoardModel: window.expansionBoardModel,
      rgbLEDModel: window.rgbLEDModel
    });
    
    switch (targetName) {
      case 'jstPin':
        const jstPinGroup = window.jstPin ? window.jstPin.group : null;
        console.log('Shader Manager: jstPin group found:', jstPinGroup);
        return jstPinGroup;
      case 'jstPin.pinGLTF1':
        return window.jstPin ? window.jstPin.pinGLTF1 : null;
      case 'jstPin.pinGLTF2':
        return window.jstPin ? window.jstPin.pinGLTF2 : null;
      case 'jstPin2':
        return window.jstPin2 ? window.jstPin2.group : null;
      case 'jstPin2.pinGLTF1':
        return window.jstPin2 ? window.jstPin2.pinGLTF1 : null;
      case 'jstPin2.pinGLTF2':
        return window.jstPin2 ? window.jstPin2.pinGLTF2 : null;
      case 'jstPin3':
        return window.jstPin3 ? window.jstPin3.group : null;
      case 'nanoModel':
        return window.nanoModel;
      case 'tempSensorModel':
        return window.tempSensorModel;
      case 'expansionBoard':
        return window.expansionBoardModel;
      case 'buzzer': {
        // Prefer a child named 'buzzer' on the expansion board
        if (window.expansionBoardModel) {
          let buzzerChild = null;
          window.expansionBoardModel.traverse((child) => {
        ...
        return window.buzzerModel || null;
      }
      case 'buzzerPin': {
      ...
      case 'tsopPin': {
        ...
        if (window.tsopModel) {
          let target = null;
          window.tsopModel.traverse((child) => {
      ...
      case 'malePin': {
        ...
        if (window.buzzerModel) {
          ...
          window.buzzerModel.traverse((child) => {
      ...
      case 'rgbPin1':
        ...
        const expansionBoard = window.expansionBoardModel;
        if (expansionBoard) {
      ...
      case 'ldrPin':
        ...
        return null;
      case 'ldrModel':
        // Return the LDR model from window
        console.log('Shader Manager: ldrModel found:', window.ldrModel);
        return window.ldrModel;
     
      case 'jstPinBattery':
        // Return the battery JST pin group if available
        return window.jstPinBattery ? window.jstPinBattery.group || window.jstPinBattery.pinGLTF1 : null;
      case 'jstPinBattery.pinGLTF1':
        return window.jstPinBattery ? window.jstPinBattery.pinGLTF1 : null;
      case 'powerPin':
      ...
      case 'motorDriverPin': {
        ...
        if (window.motorDriverModel) {
          let target = null;
          window.motorDriverModel.traverse((child) => {
            if (child && typeof child.name === 'string' && child.name.toLowerCase().includes('motordriverpin')) {
              target = child;
            }
          });
  ...
  handleDragStart(draggedObject) {
    ...
    if (onDragStartAction === 'removeFromJstPinAndApplyToRgbPin1' && (draggedObjectName === 'jstPin' || draggedObjectName === 'jstPin.pinGLTF1' || draggedObjectName === 'jstPin.pinGLTF2')) {
      ...
      console.log('Shader Manager: expansionBoardModel exists:', !!window.expansionBoardModel);
  ...
  handleDragEnd(draggedObject) {
    ...
    if (typeof window.getCurrentLesson === 'function' && window.getCurrentLesson() === 'lesson1') {
      ...
      if (window.orbitControls) {
        ...
        console.log('OrbitControls re-enabled and camera lock released after drag end for lesson1 step', getCurrentStep());
      }
    }
  ...
  handleSnap(snapType) {
    ...
    try {
      ...
      if (typeof window.enableOrbitControls === 'function') {
        ...
        if (window.orbitControls && getCurrentStep() !== 0) {
          setTimeout(() => {
            window.orbitControls.enabled = true;
            window.orbitControls.update();
  ...
  getObjectName(object) {
    ...
    if (window.jstPin && (object === window.jstPin.pinGLTF1 || object === window.jstPin.pinGLTF2)) {
      console.log('Shader Manager: Identified as jstPin child');
      return 'jstPin';
    }
    if (window.jstPin2 && (object === window.jstPin2.pinGLTF1 || object === window.jstPin2.pinGLTF2)) {
      console.log('Shader Manager: Identified as jstPin2 child');
      return 'jstPin2';
    }
    if (window.jstPin3 && (object === window.jstPin3.pinGLTF1 || object === window.jstPin3.pinGLTF2)) {
...
export function testShaderManager() {
  ...
  const jstPin = window.jstPin?.group;
  if (jstPin) {
    ...
    manager.applyShaderToObject(jstPin, testStepConfig);
