...
function startElectroCloseupRedAlertAndShake() {
  ...
  try {
    ...
    const loop = () => {
      ...
      const t = (now - redVignette.startTime) * 0.004; // speed
      const pulse = (Math.sin(t) * 0.5 + 0.5); // 0..1
      const intensity = 0.25 + pulse * 0.35; // 0.25..0.6
      ...
      gradient.addColorStop(0, `rgba(200, 0, 0, ${intensity * 0.25})`);
...
// New function to show electro and redzone after Zoe sequence completes
