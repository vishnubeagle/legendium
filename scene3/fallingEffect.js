...
let cameraOffset = new THREE.Vector3(0, 1.5, -2);
let cracked = false;
let exploded = false;
let isFalling = false;
let floor = null;
...
// Add new falling sequence variables
let isMainFallingSequenceActive = false;
let mainFallingRocks = [];
let mainFallingDebris = [];
let mainFallStartTime = 0;
let mainIsShattering = false;
let mainCrackedMeshes = new Set();
let mainShatteredPieces = [];
let mainOriginalMeshPositions = new Map();
let mainCrackMaterial = null;
...
function createTunnelEffect(centerPosition) {
    // Enhanced tunnel shader material
    tunnelShaderMaterial = new THREE.ShaderMaterial({
        uniforms: {
            time: { value: 0 },
        ...
        fragmentShader: `
            ...
            void main() {
                ...
                float pattern = sin(angle * 20.0 + time * speed) * 0.5 + 0.5;
                float pulse = sin(time * 4.0) * 0.5 + 0.5;
                
                vec3 color = mix(color1, color2, pattern);
                float alpha = smoothstep(scale, 0.0, dist) * pulse;
    ...
    // Create tunnel geometry at center position
    const tunnelGeometry = new THREE.CylinderGeometry(15, 15, 200, 64, 1, true);
    tunnelMesh = new THREE.Mesh(tunnelGeometry, tunnelShaderMaterial);
    tunnelMesh.rotation.x = Math.PI / 2;
    tunnelMesh.position.copy(centerPosition);
    tunnelMesh.position.y -= 100;
    scene.add(tunnelMesh);
    isTunnelActive = true;
}

function createNebulaParticles(centerPosition) {
    ...
    for (let i = 0; i < particleCount; i++) {
        // Create particles in a tighter radius around center position
        const radius = Math.random() * 4; // Reduced from 6 to 4
        const angle = Math.random() * Math.PI * 2;
        positions[i * 3] = centerPosition.x + Math.cos(angle) * radius;
        positions[i * 3 + 1] = centerPosition.y + Math.random() * -150; // Reduced depth
        positions[i * 3 + 2] = centerPosition.z + Math.sin(angle) * radius;

        // Enhanced nebula colors
        const color = new THREE.Color();
        color.setHSL(Math.random() * 0.2 + 0.6, 1.0, 0.8);
        colors[i * 3] = color.r;
...
function spawnParticles() {
    ...
    const dustCount = 1500; // Reduced count for less clustering
    const debrisCount = 300; // Reduced count for less clustering
    ...
    for (let i = 0; i < debrisCount; i++) {
        const particleGeometry = new THREE.OctahedronGeometry(0.04, 0); // Changed to octahedron for sharp edges
...
export function updateFallingEffect(delta, player) {
    ...
    // Update tunnel effect position
    if (tunnelMesh) {
        tunnelMesh.material.uniforms.time.value += delta;
        tunnelMesh.position.copy(centerPosition);
        tunnelMesh.position.y -= 100;
    }
