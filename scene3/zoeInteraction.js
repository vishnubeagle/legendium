  ...
  try {
    ...
    if (!autoMoveCameraOriginal.target) {
      if (currentControls && currentControls.target) {
        ...
        autoMoveCameraOriginal.target = currentCamera.position.clone().add(forward);
      }
    }
    ...
    if (currentControls && currentControls.target) {
      currentControls.target.set(electroLookAt.x, electroLookAt.y, electroLookAt.z);
        ...
        autoMoveReturnTimer = setTimeout(() => {
          ...
          gsap.to(currentCamera.position, {
            ...
            onComplete: () => {
              ...
              try { if (typeof playElectroBreathingIdleAction === 'function') playElectroBreathingIdleAction(); } catch (_) {}
              // Invoke deferred callback
              if (typeof autoMoveCallback === 'function') {
                const cb = autoMoveCallback; autoMoveCallback = null; cb();
              }
            }
          });
          if (currentControls && currentControls.target && autoMoveCameraOriginal.target) {
            gsap.to(currentControls.target, {
              ...
              duration: restoreDuration,
              ease: "power2.inOut",
              onUpdate: () => { currentControls.update && currentControls.update(); }
            });
          }
        }, 3800);
      }
    ...
    isAutoMoveCameraAnimating = false;
    if (typeof autoMoveCallback === 'function') { const cb = autoMoveCallback; autoMoveCallback = null; cb(); }
  }
}
...
function startAutoMoveCameraSequence() {
  ...
  try {
    ...
    const startTarget = (currentControls && currentControls.target)
      ? currentControls.target.clone()
      : currentCamera.position.clone().add(new THREE.Vector3(0, 0, -1).applyQuaternion(currentCamera.quaternion));
    ...
    const step = () => {
      ...
      const targetWithJitter = baseTarget.clone().add(new THREE.Vector3(jitterX, jitterY, jitterZ));
...
function stopAllElectroActions() {
  try {
    if (zoeElectroCharacter && zoeElectroCharacter.userData && zoeElectroCharacter.userData.zoeActions) {
      Object.values(zoeElectroCharacter.userData.zoeActions).forEach((action) => {
...
function playElectroWavingAction() {
  try {
    if (!zoeElectroCharacter || !zoeElectroCharacter.userData || !zoeElectroCharacter.userData.zoeActions) return;
...
/**
 * Immediately stops the Zoe sequence and triggers completion/cleanup.
 */
function skipZoeSequence() {
  ...
  try {
    ...
    if (allAssets?.audios?.electrosound1 && allAssets.audios.electrosound1.isPlaying) allAssets.audios.electrosound1.stop();
    if (allAssets?.audios?.zoesound11 && allAssets.audios.zoesound11.isPlaying) allAssets.audios.zoesound11.stop();
    if (allAssets?.audios?.zoesound1 && allAssets.audios.zoesound1.isPlaying) allAssets.audios.zoesound1.stop();
    if (allAssets?.audios?.electrosound2 && allAssets.audios.electrosound2.isPlaying) allAssets.audios.electrosound2.stop();
    if (shakeSound && shakeSound.isPlaying) shakeSound.stop();
    if (allAssets?.audios?.electroLookat && allAssets.audios.electroLookat.isPlaying) allAssets.audios.electroLookat.stop();
  } catch (_) {}
...
export function initializeZoe(scene, assets, camera, batchSystemRef, controlsRef) {
  ...
  currentPlayer = scene.getObjectByName("playerCapsule") || 
    ...
    if (interiorModel) {
      ...
      if (!tvScreenMesh) {
        interiorModel.traverse((child) => {
          if (!tvScreenMesh && child.isMesh && typeof child.name === 'string' && child.name.toLowerCase().includes('tv')) {
            tvScreenMesh = child;
          }
        });
      }
  ...
  if (!zoeActions.idleAction || !zoeActions.walkAction || !zoeActions.talkAction || !zoeActions.wcircleAction) {
    console.error("Zoe animations not found. Available animations:", Object.keys(animations));
  ...
  zoeTrigger = TriggerPoint(
    ...
    () => {
      ...
      console.log("Player has reached Zoe's location! Player position:", playerPosition);
      
      // Hide current objective during Zoe interaction
      try { hideSceneObjective(); } catch (_) {}
  ...
  if (assets.characters && assets.characters.models.electro) {
    ...
    if (assets.characters.animations.electro) {
      ...
      Object.entries(actionNames).forEach(([key, name]) => {
        const originalAction = assets.characters.animations.electro.actions[name];
        if (originalAction) {
          const newAction = zoeElectroMixer.clipAction(originalAction.getClip());
    ...
    } catch (_) { zoeShapeKeyControls = null; }
  }
...
function startZoeSequence() {
  ...
  if (zoeCharacter && currentCamera) {
    ...
    if (currentControls && currentControls.target) {
      currentCamera.userData.originalControlsTarget = currentControls.target.clone();
    }
  ...
  Object.entries(zoeActions).forEach(([name, action]) => {
    ...
    if (name !== 'idleAction') {
      ...
      if (zoeActions.walkAction) {
        ...
        const fromIdle = zoeActions.idleAction && zoeActions.idleAction.isRunning() ? zoeActions.idleAction : null;
        if (fromIdle) {
          zoeActions.walkAction.crossFadeFrom(fromIdle, animationCrossFadeDuration, false).play();
        } else {
    ...
    function updateZoePosition() {
      ...
      if (progress < 1) {
        ...
        if (zoeActions.talkAction && zoeActions.walkAction) {
          ...
          if (zoeActions.idleAction && zoeActions.idleAction.isRunning()) {
            zoeActions.idleAction.stop();
            console.log("Stopped idle animation during walk-to-talk transition");
          }
          
          zoeActions.talkAction.reset();
          zoeActions.talkAction.enabled = true;
          zoeActions.talkAction.crossFadeFrom(
  ...
  if (zoeSound) {
          ...
          setTimeout(() => {
            ...
            if (zoeElectroCharacter) {
              ...
              try { if (zoeShapeKeyControls) zoeShapeKeyControls.startAnimation(); } catch (_) {}
              
              // Play Electro's talk animation using talking action like in scene 1
              if (zoeElectroCharacter.userData.zoeActions && zoeElectroCharacter.userData.zoeActions.talking) {
                zoeElectroCharacter.userData.zoeActions.talking.reset();
                zoeElectroCharacter.userData.zoeActions.talking.setLoop(THREE.LoopRepeat);
                zoeElectroCharacter.userData.zoeActions.talking.clampWhenFinished = false;
                zoeElectroCharacter.userData.zoeActions.talking.fadeIn(0.5).play();
              }
            ...
            if (allAssets && allAssets.audios && allAssets.audios.electrosound1) {
              console.log("Playing Electro's sound");
              const electroSound = allAssets.audios.electrosound1;
              
              // Pause Zoe's current actions
              ['talkAction', 'eyesAction', 'recordAction'].forEach(actionKey => {
                if (zoeActions[actionKey] && zoeActions[actionKey].isRunning()) {
              ...
              setTimeout(() => {
                ...
                if (zoeElectroCharacter && zoeElectroCharacter.userData.originalState) {
                  ...
                  try { if (zoeShapeKeyControls) zoeShapeKeyControls.stopAnimation(); } catch (_) {}
                ...
                startEyeBlinking();
                                        ...
                                        // Play Zoe's sound11
                        if (allAssets && allAssets.audios && allAssets.audios.zoesound11) {
                              ...
                              if (hudVideoPlane) {
                                hudVideoPlane.userData.loadPromise
                                  .then(() => {
                                    ...
                                    video.currentTime = 0;
                                    video.play().catch((e) => { console.warn('TV video play failed:', e); });
                                  ...
                                  .catch(() => {
                                    if (hudVideoPlane) {
                                      try {
                                        if (hudVideoPlane.userData && hudVideoPlane.userData.originalMaterial) {
                      ...
                      setTimeout(() => {
                        ...
                        if (allAssets?.audios?.electrosound2) {
                          ...
                          if (zoeElectroCharacter) {
                            ...
                            if (zoeElectroCharacter.userData.zoeActions && zoeElectroCharacter.userData.zoeActions.SAD_TALKING) {
                              zoeElectroCharacter.userData.zoeActions.SAD_TALKING.reset();
                              zoeElectroCharacter.userData.zoeActions.SAD_TALKING.setLoop(THREE.LoopOnce);
                              ...
                              zoeElectroCharacter.userData.zoeActions.SAD_TALKING.fadeIn(0.5).play();
                              ...
                              (function() {
                                const actions = zoeElectroCharacter.userData.zoeActions;
                                ...
                                const effectiveScale = (typeof fromAction.getEffectiveTimeScale === 'function')
                                  ? Math.max(0.0001, fromAction.getEffectiveTimeScale())
                                  : 1.0;
                                const totalDurationSec = (clip?.duration || 0) / effectiveScale;
                                const startDelayMs = Math.max(0, (totalDurationSec - crossFadeDuration) * 1000);
                                ...
                                if (typeof toAction.setEffectiveTimeScale === 'function') {
                                  toAction.setEffectiveTimeScale(animationSpeedFactor);
                                }
                                if (typeof toAction.setEffectiveWeight === 'function') {
                                ...
                                const startBreathing = () => {
                                  ...
                                  if (typeof toAction.crossFadeFrom === 'function') {
                                    toAction.crossFadeFrom(fromAction, crossFadeDuration, false).play();
                                  } else {
                                    try { fromAction && fromAction.fadeOut && fromAction.fadeOut(crossFadeDuration); } catch (_) {}
                                    toAction.fadeIn(crossFadeDuration).play();
                                  }
                                };

                                const timerId = setTimeout(startBreathing, startDelayMs);

                                // Fallback: if action finishes early or timers are delayed (tab inactive), listen on mixer
                                try {
                                  if (zoeElectroMixer && typeof zoeElectroMixer.addEventListener === 'function') {
                                    const onFinished = (e) => {
                                      try {
                                        if (e && e.action === fromAction) {
                                          clearTimeout(timerId);
                                          startBreathing();
                                          zoeElectroMixer.removeEventListener('finished', onFinished);
                                        }
                                      } catch (_) {}
                                    };
                                    zoeElectroMixer.addEventListener('finished', onFinished);
                                  }
                                } catch (_) {}
                              })();
                            }
                          ...
                          setTimeout(() => {
                            if (isSequenceSkipped) return; // Skip if sequence was skipped
                            console.log("Electro's second sound finished, returning to player");
...
function endZoeSequence() {
  ...
  if (currentCamera && currentCamera.userData.originalPosition) {
    ...
    try { gsap.killTweensOf(currentCamera.position); } catch (_) {}
    if (currentControls && currentControls.target) {
      try { gsap.killTweensOf(currentControls.target); } catch (_) {}
    }
    ...
    if (currentControls && currentControls.target && currentCamera.userData.originalControlsTarget) {
      currentControls.target.copy(currentCamera.userData.originalControlsTarget);
      currentControls.update && currentControls.update();
    }
  ...
  if (currentControls) {
    ...
    try { currentControls.update && currentControls.update(); } catch (_) {}
  }
...
function createPostZoeTrigger() {
  if (!currentScene || !allAssets) {
    console.error("Scene or assets not available for creating post-Zoe trigger");
    return;
  }
  ...
  setCameraFollowsPlayer(false);
      ...
      try { if (typeof playElectroTalking02Action === 'function') playElectroTalking02Action(); } catch (_) {}
      ...
      setTimeout(() => {
        try {
          ...
          console.warn('Could not play electroLookat audio on post-Zoe trigger:', e);
        }
      }, 1200);
      ...
      triggerAutoMoveAndDisableControls(currentPlayer, () => {
        ...
        try { showSceneObjective(3); } catch (_) {}
      });
    }
  );
...
export function updateZoe(delta, player) {
  ...
  if (postZoeTrigger && typeof postZoeTrigger.updateQuarksScene === 'function') {
  ...
  if (isAutoMoving) {
    ...
    if (!isAutoMoveCameraAnimating && currentPlayer && currentCamera) {
      const forward = new THREE.Vector3(0, 0, -1).applyQuaternion(currentPlayer.quaternion).normalize();
      const camOffsetFront = 0.6;
      const camOffsetUp = 0.0;
      const desiredCamPosition = currentPlayer.position.clone()
      ...
      const lookAhead = currentPlayer.position.clone().add(forward.clone().multiplyScalar(3.0));
...
export function cleanupZoe() {
  ...
  if (hudVideoPlane) {
    ...
    if (typeof hudVideoPlane.userData._detachVideoListeners === 'function') {
      try { hudVideoPlane.userData._detachVideoListeners(); } catch (_) {}
    }
    if (hudVideoPlane.userData.videoTexture && hudVideoPlane.userData.videoTexture.dispose) {
      try { hudVideoPlane.userData.videoTexture.dispose(); } catch (_) {}
    }
    if (isTvMesh) {
      // Restore original material if stored
      if (hudVideoPlane.userData.originalMaterial) {
        // Dispose video material if created
        if (hudVideoPlane.userData.videoMaterial && hudVideoPlane.userData.videoMaterial.dispose) {
...
function startVignetteAndShake() {
  ...
  try {
    if (currentCamera) {
      ...
      preShakeCameraState.target = (currentControls && currentControls.target)
        ? currentControls.target.clone()
        : null;
      const widePos = (SHAKE_CAMERA_ANGLES && SHAKE_CAMERA_ANGLES[0]) ? SHAKE_CAMERA_ANGLES[0] : new THREE.Vector3(42, 3, -55);
      const lookAtZoe = (zoeCharacter && zoeCharacter.position)
      ...
      if (SHAKE_CAMERA_ANGLES && SHAKE_CAMERA_ANGLES.length > 1) {
        ...
        const tId = setTimeout(() => {
          ...
          const lookZoe = (zoeCharacter && zoeCharacter.position)
            ? zoeCharacter.position.clone()
            : new THREE.Vector3(82, -2.5, -60);
          try { gsap.killTweensOf(currentCamera.position); } catch (_) {}
          if (currentControls && currentControls.target) {
  ...
  shakeInterval = setInterval(() => {
    if (currentCamera) {
      ...
      if (currentShakeIntensity < maxShakeIntensity) {
        currentShakeIntensity = Math.min(currentShakeIntensity + shakeIncreaseStep, maxShakeIntensity);
      }
    }
...
function stopVignetteAndShake() {
  ...
  try {
    ...
    if (shakeAngleTimeouts && shakeAngleTimeouts.length) {
      shakeAngleTimeouts.forEach((id) => { try { clearTimeout(id); } catch (_) {} });
...
function createHUDVideoPlane() {
  ...
  if (!tvMesh) {
    interiorModel.traverse((child) => {
      if (!tvMesh && child.isMesh && typeof child.name === 'string' && child.name.toLowerCase().includes('tv')) {
        tvMesh = child;
      }
    });
  }
  ...
  try {
    ...
    const dims = [Math.max(size.x, 1e-6), Math.max(size.y, 1e-6), Math.max(size.z, 1e-6)].sort((a, b) => b - a);
...
export function startZoeIdle() {
  if (zoeActions && zoeActions.idleAction && zoeCharacter && zoeCharacter.visible) {
...
export function autoPlayerMovement(delta) {
  ...
  const moveDirection = new THREE.Vector3(0, 0, -1).applyQuaternion(currentPlayer.quaternion);
  ...
  if (autoMoveDistance >= AUTO_MOVE_CONFIG.maxDistance) {
    ...
    console.log("Calling updatePlayerAnimation(true, false) for walking animation");
