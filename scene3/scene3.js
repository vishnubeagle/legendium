...
export async function initializeScene3(existingRenderer, isVRMode) {
  ...
  ambLight = new THREE.AmbientLight("white", 0.6);
  ...
  initializeFadeOut(scene, camera);
  ...
  if (interiorModel) {
    interiorModel.traverse((child) => {
      if (child.isMesh && child.name === "polySurface5714001") {
        ...
        // Create a smooth rotation animation using GSAP
        gsap.to(child.rotation, {
          y: Math.PI * 2, // Full 360-degree rotation
          duration: 10, // 10 seconds for one complete rotation
          repeat: -1, // Infinite repeat
          ease: "none", // Linear rotation for smooth movement
          onUpdate: () => {
            // Force update the rotation
            child.rotation.z = child.rotation.z % (Math.PI * 2);
          },
        });
      }
    });
  ...
  if (interiorModel) {
    interiorModel.traverse((child) => {
      if (
        ...
        // child.userData.lineMesh = lineMesh;

        console.log("Added flowing line effect to:", child.name);
      }
    });
  }
